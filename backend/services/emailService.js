...
export async function sendEmailAlert(alert, currentValue, deviceId) {
  try {
    ...
    const emailContent = {
      ...
      subject: `🚨 Farm Alert: ${getParameterLabel(alert.parameter)} ${alert.comparison} ${alert.threshold}`,
      html: generateEmailHTML(alert, currentValue, deviceId),
      text: generateEmailText(alert, currentValue, deviceId)
    };

    console.log('📧 Sending email alert to:', alert.value);
...
function generateEmailText(alert, currentValue, deviceId) {
  ...
  return `
${alert.critical ? '🚨' : '🔔'} ${criticalText} - Smart Agriculture Monitoring System
...
function logEmailContent(alert, currentValue, deviceId) {
  ...
  console.log('📧 Email content (SendGrid not configured):', emailContent);
}
