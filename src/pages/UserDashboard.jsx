...
const UserDashboard = () => {
  ...
  // Load user data and check for assigned devices
  useEffect(() => {
    const loadUserData = async () => {
      ...
      try {
        ...
        // First, check for assigned devices in deviceRequests collection
        ...
        try {
          ...
          // Don't throw error - this is expected for new users without requests
        }

        if (assignedRequests.length > 0) {
          // Get the first assigned device
          ...
          // Fallback: check user document for devices
  ...
  // Subscribe to real-time sensor data and store in Firestore
  useEffect(() => {
    if (!deviceId) return;
  ...
  // Load device requests in real-time
  useEffect(() => {
    if (!currentUser?.uid) return;
    ...
    // Use the service method which handles the query properly
    const unsubscribe = deviceRequestsService.subscribeToUserRequests(
      ...
      (requests) => {
        ...
        // Sort by createdAt manually if needed (service might not have orderBy due to index issues)
        ...
        // Update device ID if a new device is assigned
  ...
  // Load accessible devices (owned + shared)
  useEffect(() => {
    const loadAccessibleDevices = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Set primary device (first owned device, or first shared if no owned devices)
          ...
          // Don't show error toast for API failures - this is expected if backend is not fully configured
        }
      } catch (error) {
  ...
  // Get status color for request cards
  ...
  // Check if request can be updated or cancelled
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                      ...
                      {accessibleDevices.map((device) => (
                        <DeviceCard
                          ...
                          onDeviceClick={(device) => {
                            ...
                            setActiveTab('sensors'); // Switch to sensors tab to show data
                          }}
                        />
                      ))}
                    </div>
                  ...
                  {deviceRequests.length > 0 ? (
                    <div className="space-y-4">
                      {deviceRequests.map((request) => {
                        // Use the globally defined status color function
        ...
        {showRequestModal && (
          <RequestDeviceModal
            ...
            onSuccess={() => {
              ...
              // Reload user data to check for new device assignment
        ...
        {showShareModal && selectedDevice && (
          <DeviceShareModal
            ...
            onShareSuccess={() => {
              // Could refresh device list if needed
            }}
          />
        )}
        
        {/* Recent Orders Section - Show in overview or as separate section */}
