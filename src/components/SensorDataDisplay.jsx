...
const SensorDataDisplay = ({ deviceId }) => {
  ...
  useEffect(() => {
    ...
    console.log('🔄 Setting up real-time subscription for sensor data:', deviceId);

    const unsubscribe = sensorDataService.subscribeToDeviceSensorData(
      deviceId,
      (data) => {
        console.log('📥 Received sensor data:', data.length);
        setSensorData(data);
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              Device: {deviceId} • {sensorData.length} readings
            </p>
          </div>
        </div>
      </div>
