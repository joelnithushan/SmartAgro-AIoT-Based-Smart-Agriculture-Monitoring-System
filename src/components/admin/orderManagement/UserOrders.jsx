...
const UserOrders = ({ onRequestDevice }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = deviceRequestsService.subscribeToUserRequests(
      ...
      (userOrders) => {
        ...
        // Update selectedOrderForPDF if it exists and has been updated
        if (selectedOrderForPDF) {
          const updatedOrder = userOrders.find(order => order.id === selectedOrderForPDF.id);
          if (updatedOrder) {
            setSelectedOrderForPDF(updatedOrder);
          }
        }
  ...
  }, [currentUser?.uid, selectedOrderForPDF]);
                ...
                <div className="bg-green-50 rounded-lg p-4 mb-4 border border-green-200">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xl font-bold text-green-700">
                      {formatCurrency(order.costDetails.totalCost)}
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    <div>
                      <span className="text-green-600 font-medium">Device:</span>
                      <span className="text-green-800 ml-2">{formatCurrency(order.costDetails.deviceCost)}</span>
                    </div>
                    <div>
                      <span className="text-green-600 font-medium">Service:</span>
                      <span className="text-green-800 ml-2">{formatCurrency(order.costDetails.serviceCharge)}</span>
                    </div>
                    <div>
                      <span className="text-green-600 font-medium">Delivery:</span>
                      <span className="text-green-800 ml-2">{formatCurrency(order.costDetails.deliveryCharge)}</span>
                    </div>
                  </div>
                </div>
              )}
