...
const UserActions = ({ user, onUserUpdated, onUserDeleted, onViewUser, currentUserEmail }) => {
  ...
  const canPromoteUser = (user) => {
    ...
    // Only super admin can promote users; cannot promote super admin
    return isCurrentUserSuperAdmin && user.role === 'user' && !isSuperAdmin(userEmail);
  };
  
  const canDemoteUser = (user) => {
    const userEmail = getUserEmail(user);
    // Only super admin can demote admins; cannot demote super admin
  ...
  const canDeleteUser = (user) => {
    ...
    if (isCurrentUserAdmin && user.role === 'user') return true; // Admin can delete only normal users
    return false;
  };
  ...
  const handlePromote = () => {
    ...
    setModalConfig({
      ...
      confirmText: 'Promote',
      confirmClass: 'bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 focus:ring-blue-500 shadow-lg hover:shadow-blue-500/25',
      action: 'promote',
      type: 'info'
    });
  ...
  const handleDemote = () => {
    ...
    setModalConfig({
      ...
      confirmText: 'Demote',
      confirmClass: 'bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 focus:ring-orange-500 shadow-lg hover:shadow-orange-500/25',
      action: 'demote',
      type: 'warning'
    });
    setShowConfirmModal(true);
  ...
  return (
    <>
      <div className="flex space-x-2">
        ...
        {canPromoteUser(user) && (
          <button
            className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        {canDemoteUser(user) && (
          <button
            className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-orange-500"
        ...
        {canDeleteUser(user) && (
          <button
            className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-md hover:shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-red-500"
