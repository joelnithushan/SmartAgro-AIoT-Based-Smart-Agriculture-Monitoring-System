  ...
  useEffect(() => {
    ...
    const unsubscribeLatest = onValue(latestRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        if (normalizedData) {
          ...
          // If we're receiving sensor data, device must be online
          setIsOnline(true);
    ...
    const offlineCheckInterval = setInterval(() => {
      if (window.lastESP32UpdateTime) {
        ...
        if (timeSinceLastUpdate > OFFLINE_TIMEOUT && isOnline) {
          console.log('📴 Device went offline - no updates for', Math.round(timeSinceLastUpdate/1000), 'seconds');
          setIsOnline(false);
          zeroSensorData();
        }
      }
