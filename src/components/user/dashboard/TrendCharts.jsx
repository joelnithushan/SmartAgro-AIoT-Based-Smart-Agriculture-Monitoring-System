    ...
    const unsubscribe = onValue(historyRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const historyArray = Object.entries(data || {})
          .map(([key, item]) => {
            // Handle timestamp properly - try multiple sources
            ...
            return {
              ...
              airTemperature: item.airTemperature || item.air_temperature || item.temperature || 
                             item.airTemp || item.temp || 0,
              airHumidity: item.airHumidity || item.air_humidity || item.humidity || 
                          item.humidityPct || item.humidity_pct || 0,
              soilTemperature: item.soilTemperature || item.soil_temperature || item.soilTemp || 
                              item.soil_temp || 0,
              ...
              rainLevel: item.rainLevelRaw || item.rain_level_raw || item.rainSensor || 
