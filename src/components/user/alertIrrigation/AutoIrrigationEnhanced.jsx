...
const AutoIrrigationEnhanced = () => {
  const { user } = useAuth();
  const { activeDeviceId } = useUserDevices();
  const { sensorData, isOnline } = useDeviceRealtime(activeDeviceId);
  ...
  // Auto irrigation logic - check every 30 seconds when enabled
  useEffect(() => {
    if (!autoIrrigationEnabled || !isOnline || !sensorData?.soilMoisturePct || !activeDeviceId) {
      return;
    }

    const checkAutoIrrigation = async () => {
      try {
        const moisture = sensorData.soilMoisturePct;
        
        // Determine if pump should be on or off
        let shouldBeOn = false;
        let reason = '';
        
        if (moisture < settings.turnOnThreshold) {
          shouldBeOn = true;
          reason = `Soil moisture (${moisture.toFixed(1)}%) below turn-on threshold (${settings.turnOnThreshold}%)`;
        } else if (moisture > settings.turnOffThreshold) {
          shouldBeOn = false;
          reason = `Soil moisture (${moisture.toFixed(1)}%) above turn-off threshold (${settings.turnOffThreshold}%)`;
        }

        // Check if action is needed (avoid duplicate actions)
        const currentPumpStatus = sensorData.relayStatus || 'off';
        const actionNeeded = (shouldBeOn && currentPumpStatus !== 'on') || (!shouldBeOn && currentPumpStatus !== 'off');

        if (actionNeeded && reason) {
          console.log(`🤖 Auto irrigation action needed: ${shouldBeOn ? 'ON' : 'OFF'} - ${reason}`);
          
          // Call auto irrigation endpoint
          const response = await fetch(`http://localhost:5000/api/irrigation/auto-control/${activeDeviceId}`, {
            method: 'POST',
            headers: {
          ...
          if (response.ok) {
            ...
            if (result.action) {
              toast.success(`Auto irrigation: Pump turned ${result.action} - ${result.reason}`);
            }
          }
        }
      } catch (error) {
        console.error('Error in auto irrigation check:', error);
      }
    };
    ...
    const interval = setInterval(checkAutoIrrigation, 30000); // Check every 30 seconds

    return () => clearInterval(interval);
  }, [autoIrrigationEnabled, isOnline, sensorData?.soilMoisturePct, settings, activeDeviceId]);
  ...
  const getPumpAction = () => {
    if (!autoIrrigationEnabled || !isOnline || sensorData.soilMoisturePct === undefined) {
      return null;
    }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h3 className="text-lg font-medium text-gray-900 mb-4">Enhanced Auto Irrigation Control</h3>
        ...
        {lastAutoAction && (
          <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-md">
            <p className="text-sm text-green-800">
              <span className="font-medium">Last Auto Action:</span> {lastAutoAction.action} - {lastAutoAction.reason}
            </p>
            <p className="text-xs text-green-600">
              {lastAutoAction.timestamp.toLocaleTimeString()}
            </p>
          </div>
        )}

        {/* Warning Messages */}
        {!isOnline && autoIrrigationEnabled && (
      ...
      <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
        <h4 className="text-sm font-medium text-green-900 mb-2">How Enhanced Auto Irrigation Works</h4>
        <ul className="text-xs text-green-800 space-y-1">
          <li>• Monitors soil moisture levels every 30 seconds when enabled</li>
          ...
          <li>• Updates are sent directly to your ESP32 device via Firebase Realtime Database</li>
...
export default AutoIrrigationEnhanced;
